...
function login() {
    ...
    //    // Si el usuario está bloqueado, mostrar mensaje y salir de la función
    //    var bloqueo = JSON.parse(localStorage.getItem("bloqueo_" + USR_RUT));
    //    if (bloqueo && bloqueo.finBloqueo > Date.now()) {
    //        var tiempoRestante = Math.ceil((bloqueo.finBloqueo - Date.now()) / 1000 / 60);
    //        document.getElementById("divMensaje2").innerHTML = "Ha excedido el número máximo de intentos fallidos. Este acceso será bloqueado por " + tiempoRestante + " minutos.";
    //        document.getElementById("divMensaje2").style.display = "block";
    //        return;
    //    }

    //    // Si los valores no coinciden, bloquear al usuario durante un minuto
    //    if (cli_autoid !== hdRsp) {
    //        document.getElementById("divMensaje2").innerHTML = "Datos ingresados inválidos.<br>Este acceso será bloqueado por un minuto.";
    //        document.getElementById("divMensaje2").style.display = "block";

    //        // Registrar el bloqueo del usuario en el almacenamiento local
    //        var nuevoBloqueo = {
    //            finBloqueo: Date.now() + 60 * 1000 // bloqueo por un minuto
    //        };
    //        localStorage.setItem("bloqueo_" + USR_RUT, JSON.stringify(nuevoBloqueo));

    //        // Deshabilitar los controles durante un minuto
    //        var controles = document.querySelectorAll("input,select,button");
    //        for (var i = 0; i < controles.length; i++) {
    //            controles[i].disabled = true;
    //        }

    //        // Establecer un temporizador para habilitar los controles después de un minuto
    //        setTimeout(function () {
    //            for (var i = 0; i < controles.length; i++) {
    //                controles[i].disabled = false;
    //            }
    //            localStorage.removeItem("bloqueo_" + USR_RUT);
